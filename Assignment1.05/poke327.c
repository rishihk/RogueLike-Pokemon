...
void battle()
{
    ...
    mvprintw(0, 1, "May the battle begin!");
    mvprintw(5, 1, "Press Esc to exit");
...
void building()
{
    ...
    mvprintw(0, 1, "Welcome to the Pokemon Building. How may I serve you?");
    mvprintw(5, 1, "Press < to exit");
...
void render_trainer_list()
{
    ...
    for (y = 0; y < MAP_Y; y++)
    {
        for (x = 0; x < MAP_X; x++)
        {
            if (world.cur_map->cmap[y][x] && world.cur_map->cmap[y][x]->symbol != '@')
            {
                if (y >= pcY)
                {
                    yDif = y - pcY;
                ...
                mvprintw(lineCount, 1, "%c, %d %s, %d %s \n", world.cur_map->cmap[y][x]->symbol, yDif, vert, xDif, horiz);
                lineCount++;
            }
        }
    }
    ...
    while (flag)
    {
        ...
        if (c == &world.pc)
        {
            ...
            while (!valid)
            {
                ...
                case ('y'):
                    if (world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1] && !world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1]->defeated)
                    {
                        battle();
                        world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1]->defeated = 1;
                        valid = 1;
                    }
                    else if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1]] != INT_MAX && !world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1])
                    {
                        d[dim_x] = c->pos[dim_x];
                        d[dim_y] = c->pos[dim_y];
                        world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
                        world.cur_map->cmap[d[dim_y] - 1][d[dim_x] - 1] = c;
                        c->pos[dim_y] = d[dim_y] - 1;
                        c->pos[dim_x] = d[dim_x] - 1;
                        pathfind(world.cur_map);
                        valid = 1;
                        clear();
                        render_world();
                ...
                case ('k'):
                    if (world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]] && !world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]]->defeated)
                    {
                        battle();
                        world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]]->defeated = 1;
                        valid = 1;
                    }
                    else if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x]]] != INT_MAX && !world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]])
                    {
                        d[dim_x] = c->pos[dim_x];
                        d[dim_y] = c->pos[dim_y];
                        world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
                        world.cur_map->cmap[d[dim_y] - 1][d[dim_x]] = c;
                        c->pos[dim_y] = d[dim_y] - 1;
                        c->pos[dim_x] = d[dim_x];
                        pathfind(world.cur_map);
                        valid = 1;
                ...
                case ('l'):
                    if (world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1] && !world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1]->defeated)
                    {
                        battle();
                        world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1]->defeated = 1;
                        valid = 1;
                    }
                    else if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] + 1]] != INT_MAX && !world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] + 1])
                    {
                        d[dim_x] = c->pos[dim_x];
                        d[dim_y] = c->pos[dim_y];
                        world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
                        world.cur_map->cmap[d[dim_y]][d[dim_x] + 1] = c;
                        c->pos[dim_y] = d[dim_y];
                        c->pos[dim_x] = d[dim_x] + 1;
                        valid = 1;
                        pathfind(world.cur_map);
                ...
                case ('>'):
                    if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_mart || world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_center)
                    {
                        building();
                        valid = 1;
                    }
                    ...
                    {
                        clear();
                        mvprintw(0, 0, "You are not on a PokeMart or PokeCenter \n");
                        render_world();
                    }
                    break;
